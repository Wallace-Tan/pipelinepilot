...
`your_connection_name` must be a dedicated non-production, read-only CoCo connection for the Snowflake account that owns this deployment. PipelinePilot passes it explicitly to every CoCo invocation; it does not use the active CoCo profile. Replace the placeholder with the connection name chosen by the deployment owner and verify the account binding with `cortex.cmd connections list`.
