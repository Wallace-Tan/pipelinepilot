...
Current environment evidence: the installed Cortex CLI and flattened local Airflow project have been exercised with a deployment-specific non-production connection. The read-only metadata preflight passes, but the full headless CoCo decision response is not yet schema-valid, so the live path remains unverified and must not be claimed in the submission. Deployments must supply their own named connection and expected Snowflake account.
